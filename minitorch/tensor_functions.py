...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        unit_tensor = tensor([1.0])
        return grad_output.f.mul_zip(
            grad_output, sigmoid_tensor.f.mul_zip(sigmoid_tensor, unit_tensor - sigmoid_tensor)
        )
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        order = order.to_numpy().astype(int).tolist()
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        reconstructed_order = np.argsort(order)
        return grad_output._new(grad_output._tensor.permute(*reconstructed_order)), 0.0
